...
public class Player extends Enity {
    ...
    private void checkPlatforms() {
        for (int i = 0; i < platforms.size(); i++) {
            ...
            if (!up && this.y + ySpeed - platform.getY() >= -64 && this.y + ySpeed / 4 - platform.getY() <= -48
                    && x + 8 < platform.getX() + platform.getWidth() && x + 56 > platform.getX()) {
                standOnPlatform = i;
                y = platform.getY() - 64;
    ...
    private void checkStones() {
        for (int i = 0; i < stones.size(); i++) {
            ...
            if (Math.abs(x + xSpeed - stone.getX()) <= 132 && Math.abs(y + ySpeed - stone.getY()) <= 132) {
                if (!up && this.y + ySpeed - stone.getY() >= -64 && this.y + ySpeed / 4 - stone.getY() <= -48
                        && Math.abs(this.x + xSpeed - stone.getX()) <= 48) {
                    standOnStone = i;
                    speedCarriedByStone = stone.getxSpeed();
                    stone.setySpeed(0);
                    y = stone.getY() - 64;
                } else if (this.y + ySpeed - stone.getY() <= 64 && this.y + ySpeed - stone.getY() > 0
                        && Math.abs(this.x + xSpeed - stone.getX()) <= 48) {
                    if (standOnDoor) {
                        death = true;
                    } else {
    ...
    private void checkDoor() {
        if (!up && this.y + ySpeed - door.getyHitBox() >= -64 && this.y + ySpeed / 4 - door.getyHitBox() <= -48
                && Math.abs(this.x + xSpeed - door.getX()) < 48) {
            standOnDoor = true;
            y = door.getyHitBox() - 64;
        } else if (y + 64 > door.getyHitBox() && y < door.getY() + door.getHeight()
                && Math.abs(this.x + xSpeed - door.getX()) <= 48) {
    ...
    private void checkStrangeDoor() {
        if (Math.abs((x + 32) - (strangeDoor.getX() + 64)) < 48
                && Math.abs((y + 32) - (strangeDoor.getY() + 64)) < 48) {
            nextLevel = true;
        }
    }
    ...
    private void checkTraps() {
        for (Trap trap : traps) {
            if (trap.getHeightDangerHitbox() > 0 && trap.getWidthDangerHitbox() > 0) {
                ...
                if (distanceOx < (width + trap.getWidthDangerHitbox()) / 2
                        && distanceOy < (height + trap.getHeightDangerHitbox()) / 2) {
                    death = true;
                }
            }
        }
        ...
        for (int i = 0; i < enemies.size(); i++) {
            ...
            if (enemy.isDeath() == false) {
                if (enemy instanceof Enemy1 || enemy instanceof Enemy2) {
                    if (attacking2 == true) {
                        setSwordPos();
                        if (Math.abs(xSword - enemy.getX()) < 32
                                && Math.abs((ySword + height / 2) - (enemy.getY() + enemy.getHeight() / 2)) < 32) {
                            if (enemy.isHit() == false) {
                                enemy.setHit(true);
                            }
                        }
                    } else {
                        if (Math.abs(x + xSpeed - enemy.getX()) <= 48 && y < enemy.getY() + enemy.getHeight()
                                && y + height > enemy.getY()) {
                    ...
                    if (attacking2 == true) {
                        ...
                        if (Math.abs(x + xSpeed - enemy.getX()) <= 48 && y + ySpeed < enemy.getY() + enemy.getHeight()
                                && y + ySpeed + height >= enemy.getY()) {
    ...
    protected void updatePos() {
        ...
        xSpeed = playerSpeedX + brakingSpeedByStone + brakingSpeedByDoor + speedCarriedByEnemy + speedCarriedByPlatform
                + speedCarriedByStone;
    ...
    private void setInAir(float x, float y, int[][] mapData) {
        if ((!isSolid(x + 16, y + 64, mapData)) && (!isSolid(x + 48, y + 64, mapData)) && standOnStone == -1
                && standOnDoor == false && standOnMushRoom == -1 && standOnPlatform == -1) {
            inAir = true;
        } else {
            inAir = false;
        }
