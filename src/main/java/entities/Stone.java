...
public class Stone extends Enity {
    ...
    private void checkPlatforms() {
        for (int i = 0; i < platforms.size(); i++) {
            ...
            if (ySpeed >= 0 && this.y + ySpeed - platform.getY() >= -64 && this.y + ySpeed / 4 - platform.getY() < -48
                    && x < platform.getX() + platform.getWidth() && x + width > platform.getX()) {
                y = platform.getY() - 64;
    ...
    private void checkEnimies() {
        for (int i = 0; i < enemies.size(); i++) {
            ...
            if (ySpeed > 0 && Math.abs(x - enemy.getX()) <= 64 & y < enemy.getY()
                    && Math.abs(y - enemy.getY()) < height) {
                enemy.setDeath(true);
            } else if (enemy instanceof Enemy4 && Math.abs(x + xSpeed - enemy.getX()) <= 64
                    && Math.abs(y - enemy.getY()) < height) {
                nextMushRoom4 = i;
            } else {
                if (nextMushRoom4 == i) {
